...
namespace RLib.Utils {
    static class Easing
    {
        ...
        {
            max -= min;
            return -max * Mathf.Cos(t * (Mathf.PI / 2) / totaltime) + max + min;
        }
        public static float OutSine(float t, float totaltime, float max, float min)
        {
            max -= min;
            return max * Mathf.Sin(t * (Mathf.PI / 2) / totaltime) + min;
        }
        ...
        public static float InElastic(float t, float total, float max, float min) {
            if (t == 0) return min;
            if (t == total) return max;
            t = t / total;
            float k = - Mathf.Pow(2, 8 * (t - 1)) * Mathf.Sin(((t - 1) * 80 - 7.5f) * Mathf.PI / 15);
            return min + (max - min) * k;
        }

        public static float OutElastic(float t, float total, float max, float min) {
            return max - InElastic(total - t, total, max-min, 0);
        }
    }
}
